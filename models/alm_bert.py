...
class ALMBert(nn.Module):
  ...
  def __init__(self, n_classes):
    ...
    self.ctx_att = nn.MultiheadAttention(embed_dim=self.bert.config.hidden_size, num_heads=4, dropout=0.3, batch_first=True)
    self.ctx_bn1 = nn.BatchNorm1d(768)
    self.ctx_lin = nn.Linear(768, 64)
    self.ctx_drp = nn.Dropout(0.3)
    self.ctx_bn2 = nn.BatchNorm1d(64)
    self.ctx_avg = nn.AvgPool1d(64)

    self.asp_att = nn.MultiheadAttention(embed_dim=self.bert.config.hidden_size, num_heads=4, dropout=0.3, batch_first=True)
    self.asp_bn1 = nn.BatchNorm1d(768)
    self.asp_lin = nn.Linear(768, 64)
    self.asp_drp = nn.Dropout(0.3)
  ...
  def forward(self, text_ids, text_attention, aspect_ids, aspect_attention):
    ...
    asp_out = self.asp_avg(torch.swapaxes(asp_out, 1, 2)).squeeze()
    ...
    ctx_out = self.ctx_avg(torch.swapaxes(ctx_out, 1, 2)).squeeze()
